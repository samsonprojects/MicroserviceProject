...
namespace CustomerApi.Validators.v1
{
    public class CreateCustomerModelValidator : AbstractValidator<CreateCustomerModel>
    {
        ...
        {
            RuleFor(x=> x.FirstName).NotNull().WithMessage("The First name must be at least 2 characters long");
            RuleFor(x=> x.FirstName).MinimumLength(2).WithMessage("The first name must be at least 2 characters long");
            RuleFor(x => x.LastName).NotNull().WithMessage("Last name must be at least 2 characters");
